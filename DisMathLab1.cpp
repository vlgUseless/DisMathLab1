...
int main()
{
    ...
    cout << "Enter the bit depth of the Universal set (it must be a positive integer number between 0 and 6): ";
    int n;
    cin >> n;
    while (1)
    {
        ...
        if (n < 0) {
            cout << "The bit depth must be a positive value." << endl;
            cout << "Please enter the valid input: ";
            cin >> n;
        }
        if (n > 6) {
            cout << "There's no need to create 2^" << n << " elements. Try a smaller number: ";
            cin >> n;
        }
        if (!cin.fail() and n >= 0 and n <= 6)
            break;
    }
    int sz = 1 << n;
    multiset Uni(sz), A, B;
